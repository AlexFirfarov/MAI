...
int main(int argc, char* argv[]) {
	...
	std::string out;
	double max_err, err, cur_err;
	double hx, hy, hz;
	double eps, lx, ly, lz, bc_down, bc_up, bc_left, bc_right, bc_front, bc_back, u_0;
	double *data, *temp, *next, *buff, *recvbuf_err, *errors;
	...
	errors = (double*)malloc(nx * sizeof(double));
	...
	for (;;) {
		...
		if (ib + 1 < nbx) {
			#pragma omp parallel private(j, k) shared(buff, data, ib, jb, kb, nx, ny, nz)
			{
				for (j = omp_get_thread_num(); j < ny; j += omp_get_num_threads())
					for (k = 0; k < nz; ++k)
		...
		if (jb + 1 < nby) {
			#pragma omp parallel private(i, k) shared(buff, data, ib, jb, kb, nx, ny, nz)
			{
				for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
					for (k = 0; k < nz; ++k)
		...
		if (kb + 1 < nbz) {
			#pragma omp parallel private(i, j) shared(buff, data, ib, jb, kb, nx, ny, nz) 
			{
				for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
					for (j = 0; j < ny; ++j)
		...
		if (ib + 1 < nbx) {
			...
			#pragma omp parallel private(j, k) shared(nx, ny, nz, data, buff) 
			{
				for (j = omp_get_thread_num(); j < ny; j += omp_get_num_threads())
					for (k = 0; k < nz; ++k)
			...
			#pragma omp parallel private(j, k) shared(nx, ny, nz, data, bc_right) 
			{
				for (j = omp_get_thread_num(); j < ny; j += omp_get_num_threads())
					for (k = 0; k < nz; ++k)
		...
		if (ib > 0) {
			...
			#pragma omp parallel private(j, k) shared(nx, ny, nz, data, bc_left) 
			{
				for (j = omp_get_thread_num(); j < ny; j += omp_get_num_threads())
					for (k = 0; k < nz; ++k)
		...
		if (jb + 1 < nby) {
			...
			#pragma omp parallel private(i, k) shared(nx, ny, nz, data, buff)
			{
				for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
					for (k = 0; k < nz; ++k)
			...
			#pragma omp parallel private(i, k) shared(nx, ny, nz, data, bc_back)
			{
				for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
					for (k = 0; k < nz; ++k)
		...
		if (jb > 0) {
			...
			#pragma omp parallel private(i, k) shared(nx, ny, nz, data, bc_front)
			{
				for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
					for (k = 0; k < nz; ++k)
		...
		if (kb + 1 < nbz) {
			...
			#pragma omp parallel private(i, j) shared(nx, ny, nz, data, buff)
			{
				for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
					for (j = 0; j < ny; ++j)
			...
			#pragma omp parallel private(i, j) shared(nx, ny, nz, data, bc_up)
			{
				for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
					for (j = 0; j < ny; ++j)
		...
		if (kb > 0) {
			...
			#pragma omp parallel private(i, j) shared(nx, ny, nz, data, bc_down)
			{
				for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
					for (j = 0; j < ny; ++j)
		...
		#pragma omp parallel private(i, j, k, cur_err) shared(data, next, errors, nx, ny, nz, hx, hy, hz) 
		{
			for (i = omp_get_thread_num(); i < nx; i += omp_get_num_threads())
				for (j = 0; j < ny; ++j)
					for (k = 0; k < nz; ++k) {
						...
						cur_err = std::fabs(next[_i(i, j, k)] - data[_i(i, j, k)]);
						if (cur_err > errors[i])
							errors[i] = cur_err;
					}
		}
